...
for t=minP:0.010:maxP
    ...
    sumClass0=0;
    for rowel=1:rowelNum
        %Find the value of X which is the randoem variable
        x=(r0(rowel)-1) * columns + c0(rowel);
        sumClass0=sumClass0 + (x *BPMM(r0(rowel),c0(rowel)));
    end
    ...
    for rowel=1:rowelNum
        %Find the value of X which is the randoem variable
        x=(r1(rowel)-1) * columns + c1(rowel);
        sumClass1=sumClass1 + (x * BPMM(r1(rowel),c1(rowel)));
    end
    ...
    for rowel=1:rowelNum
        %Find the value of X which is the randoem variable
        x=(r0(rowel)-1) * columns + c0(rowel);
        varClass0=varClass0 + ((x *BPMM(r0(rowel),c0(rowel)))^2-sumClass0^2);
    end
    ...
    for rowel=1:rowelNum
        %Find the value of X which is the randoem variable
        x=(r1(rowel)-1) * columns + c1(rowel);
        varClass1=varClass1 + ((x *BPMM(r1(rowel),c1(rowel)))^2-sumClass1^2);
    end
        
    mean=(sumClass0-sumClass1)^2;
    
    finalResult=mean/(varClass0+varClass1);
